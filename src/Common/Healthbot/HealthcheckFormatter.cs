...
namespace Healthbot {
    public class HealthcheckFormatter {
        ...
        public string Render(string Environment, Healthcheck healthcheck) {
            var builder = new StringBuilder();

            var aggregateState = GetOverallState(healthcheck.Checks);
            var aggregateStateText = emojiLookup[aggregateState];
            builder.AppendLine(
                $"{Environment} is running version {healthcheck.Version} and has a status of {aggregateStateText}.");
